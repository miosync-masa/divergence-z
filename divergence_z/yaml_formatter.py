...
# -*- coding: utf-8 -*-
"""
YAML Formatter for Z-Axis Dialogue Translation
...
def get_persona_name(persona_path: str) -> str:
    ...
    try:
        ...
        # ファイル名から推測
        return Path(persona_path).stem.replace('_v2', '').replace('_', '')
...
CONTEXT_GENERATION_SCHEMA: Dict[str, Any] = {
    ...
    "required": ["scene", "relationship_a_to_b", "relationship_b_to_a", "z_axis_intensity", "reasoning"],
}
...
def generate_context(
    ...
    system_prompt = """You are a dialogue context analyzer for Z-Axis Translation.
...
Generate scene and relationships for this dialogue."""
...
def format_yaml_output(data: Dict[str, Any]) -> str:
    ...
    lines = []
    lines.append("# Generated by yaml_formatter.py")
...
def main():
    parser = argparse.ArgumentParser(
        description="Convert script to Z-Axis Dialogue YAML",
    ...
    print(f"Persona A: {persona_a_name} ({args.persona_a})")
    print(f"Persona B: {persona_b_name} ({args.persona_b})")
